...
name: proxmox-ops
description: Proxmox VE operations engineer handling hypervisor lifecycle, VM orchestration, and backup integrity on the local server stack
model: sonnet
color: blue
id: proxmox-ops
summary: Maintain Proxmox cluster health, automate VM deployments, coordinate snapshots, and ensure secure remote access via Tailscale and Cloudflare.
status: active
owner: ops
...
# Proxmox Operations Playbook

#ops/infrastructure #virtualization #runbook #class/runbook

## Mission

Ensure uptime and manageability of the local Proxmox environment powering edge services (Whisper, Ollama, automation containers) while enforcing security and backup standards.

## Responsibilities

- Monitor node health (CPU, RAM, storage, ZFS) and remediate alerts.
- Provision, clone, migrate VMs/containers.
- Coordinate snapshot schedules and offsite replication to QNAP/OCI.
- Manage access: Tailscale ACLs, Cloudflare tunnels.
- Track hardware events (SMART, fans) and escalate failures.

## Key Workflows

1. **Deploy VM/CT** – Validate template & network; use Proxmox API (`qm`​, `pct`, or REST) to clone/configure; register asset in inventory.
2. **Patch Cycle** – Run monthly updates (`apt`, Proxmox VE repo), reboot via live migration if necessary.
3. **Snapshot & Backup** – Daily snapshots for critical workloads; weekly ZFS send to NAS; verify restores monthly.
4. **Incident Response** – Trigger failover, gather diagnostics, log incident in Docmost & Migration Activity Log[^1].
5. **Harbor DNS/Proxy Hygiene** – Maintain restricted `ddns.harbor.fyi`​; update `/etc/dnsmasq.d/harbor.conf`​ on NPM LXC and rotate Porkbun keys per log entry MCP Estate Update[^30]).

## Reference Tools

- Proxmox API (`https://pve.local:8006/api2/json`​), `qm`​, `pct`​, `pveperf`.
- Tailscale ACL configs, Cloudflare DNS automation scripts.

## Guardrails

- Validate backups before destructive changes.
- Enforce least privilege automation tokens.
- Document all actions in Migration Activity Log[^1] and relevant runbooks.
...
[^1]: # migration-tasks
    ...
    ## 2025-10-22 – Dashboard IA Draft
     ...
     Captured the high-level layout, card structure, and data sources for dash.aienablement.academy within Docmost &amp; NocoDB Migration Programme[^3] under “Dashboard Information Architecture (Current Concept)” to guide the next UI iteration.
    ...
    ## 2025-11-04 – Cortex Agent Enablement
     ...
     Authored Cortex (SiYuan) Operations Agent[^9] and associated skills (cortex-task-log, cortex-notebook-curation). Extended mcp/cortex server with document creation, append, attribute update, and SQL query tools. Scaffolded plugins/cortex-ops with mirrored agent/skill docs and logging commands; published plugin manifest for Claude/Codex installs. Refreshed Cortex (SiYuan) Operations &amp; Usage Guide[^10] with MCP tool guidance and synced plugins via scripts/sync/claude-sync.sh. Verified MCP toolchain end-to-end via local stub test (create doc → append block → set attrs → SQL query). Added launcher script scripts/mcp/cortex-mcp.sh and registered the server in Claude configs; secrets load from ~/.config/cortex-mcp.env to expose the tools to Claude clients.

    ## 2025-11-03 – Cortex Governance Upgrade

    - Published Cortex Metadata Standards[^12] and Tag &amp; Hashtag Taxonomy[^13] to codify second-brain metadata and tagging.
...
[^2]: # docmost-nocodb-lessons
    ...
    ## Related Knowledge
     ...
     See Docmost &amp; NocoDB Migration Programme[^3] for the end-to-end project blueprint. Cross-check backup details in Docmost &amp; NocoDB Backup SOP[^4]. Reference field diary entries in Migration Activity Log[^1].

    #docmost #nocodb #postmortem #ops


[^3]: # docmost-nocodb-plan
...
[^6]: # digitalocean-operations
    ...
    # DigitalOcean Operations Guide
     ...
     Related playbooks: DevOps Operations Guide[^7], Docker Host Operations Guide[^8], Network Engineering Playbook[^11], Docmost &amp; NocoDB Backup SOP[^4].
    ...
    ## Networking
     ...
     VPC: isolate workloads per environment; record CIDRs in Network Engineering Playbook[^11]. Floating IPs: attach for HA; update DNS accordingly. Monitoring: enable DO metrics agent; integrate with Uptime Kuma.
...
[^7]: # devops-operations
    ...
    # DevOps Operations Guide
     ...
     Related playbooks: DigitalOcean Operations Guide[^6], Docker Host Operations Guide[^8], Cortex (SiYuan) Operations Agent[^9], Docmost &amp; NocoDB Migration Programme[^3].

    #devops #ops #automation #cicd

    ## Core Responsibilities

     Own CI/CD, configuration management, and observability for all self-hosted services. Enforce infrastructure-as-code (Terraform or Ansible) for OCI, DigitalOcean, and future clouds. Maintain environment parity (dev → staging → prod) and document release cadences in Migration Activity Log[^1].
...
[^8]: # docker-host-operations
    ...
    > Related: DevOps Operations Guide[^7], Docmost &amp; NocoDB Backup SOP[^4], Migration Activity Log[^1], Docmost &amp; NocoDB Migration Programme[^3].
    >
    ...
    - Upgrades: `sudo docker compose pull && sudo docker compose up -d`​; log in Migration Activity Log[^1].
    - Backups: leverage scripts in `ops/backup/`​ (see Docmost &amp; NocoDB Backup SOP[^4]) and confirm nightly timers succeed.
    ...
    - Record every significant change in Migration Activity Log[^1] and trackers like Docmost &amp; NocoDB Project Tracker[^5] with:
...
[^9]: # cortex-siyuan-ops
    ...
    # Cortex (SiYuan) Operations Agent
     ...
     Start with Cortex (SiYuan) Operations &amp; Usage Guide[^10] and Migration Activity Log[^1] for system context.
    ...
    ## Runbook Links
     ...
     Cortex (SiYuan) Operations &amp; Usage Guide[^10] Docmost &amp; NocoDB Migration Programme[^3] scripts/sync/siyuan-export.py


[^10]: # cortex-siyuan-system
    ...
    # Cortex (SiYuan) Operations & Usage Guide
     ...
     Linked resources: Cortex (SiYuan) Operations Agent[^9], Docmost &amp; NocoDB Migration Programme[^3], Migration Activity Log[^1]
...
[^11]: # network-engineering-playbook
...
[^12]: # metadata-standards
    ...
    Document updates to this standard in Migration Activity Log[^1] and notify stewards in Docmost.
...
[^13]: # tag-taxonomy
    ...
    |​`#ops/automation`|MCP, scripts, n8n flows|[[Tools & Scripts Overview]], cortex-task-log[^14]|
    ...
    Update taxonomy changes in Migration Activity Log[^1] and ensure runbooks adopt the new tags during their next review.
...
[^14]: # cortex-task-log
    ...
    # Cortex Task Log Skill
     ...
     Operated by Cortex (SiYuan) Operations Agent[^9] and referenced in Cortex (SiYuan) Operations &amp; Usage Guide[^10].
...
[^16]: # knowledge-freshness
    ...
    Use this dashboard during weekly reviews to assign refresh tasks and log updates in Migration Activity Log[^1].
...
[^18]: # metadata-check-template
    ...
    - [ ] Hashtags added in first paragraph using taxonomy from Tag &amp; Hashtag Taxonomy[^13].
...
[^19]: # knowledge-base-overview
    ...
    ## Core Playbooks
     ...
     Network Engineering Playbook[^11] – Subnetting, DNS, TLS, and connectivity guardrails. System Architecture Guidelines[^20] – Platform principles and reference architecture. SQL &amp; PostgreSQL Playbook[^21] – Database administration, tuning, and recovery guidance. Docmost &amp; NocoDB Lessons Learned[^2] – Field notes and caveats from the migration programme. Cortex (SiYuan) Operations &amp; Usage Guide[^10] – How the second brain is structured and automated.

    ## Projects & Programmes

     Docmost &amp; NocoDB Migration Programme[^3] – End-to-end plan for the documentation/data platform transition. Docmost &amp; NocoDB Backup SOP[^4] – Nightly/weekly/monthly backup procedures and scripts. Docmost &amp; NocoDB Project Tracker[^5] – Thin-slice progress and follow-ups. Infrastructure &amp; Operations Inventory[^22] – Estate map across OCI, Proxmox, DigitalOcean, SaaS. Cloudflare Workers MCP Architecture[^23] – Edge automation strategy and fallback design. Multi-Environment Infrastructure Operations Expansion[^24] – Roadmap for unified ops coverage. Projects Dashboard[^15] – Active initiative tracker. Knowledge Freshness Dashboard[^16] – Review cadence board. Automation Inventory Dashboard[^17] – Script and MCP coverage summary.

    ## Operational Logs

     Migration Activity Log[^1] – Chronological record of stack changes, incidents, and validations.
    ...
    - Cortex Metadata Standards[^12] – Required front matter, attributes, and review cadence.
    - Tag &amp; Hashtag Taxonomy[^13] – Controlled hashtags and domain namespaces.
...
[^26]: # oci-operations
    ...
    - Snapshot instance shape/boot volume ocids after major changes; note in Migration Activity Log[^1].
    ...
    - Tracker: Docmost &amp; NocoDB Project Tracker[^5].
    - Logbook: Migration Activity Log[^1].
...
[^27]: # tailscale-operations
    ...
    - Verify: `tailscale status --json | jq '.Self.TailscaleIPs'`​; log IP + tag in Migration Activity Log[^1].
...
[^28]: # uptime-kuma-operations
    ...
    - Backup: export config via UI; include volume in Docmost &amp; NocoDB Backup SOP[^4].
...
[^29]: # formbricks-operations
    ...
    - Include `formbricks_db_data`​ in nightly backup (see Docmost &amp; NocoDB Backup SOP[^4]).
...
[^30]:  Cloned and built mcp-servers/codex-subagents-mcp, then registered it with Codex CLI, Claude Code, and Claude Z.AI (Node entrypoint + agents dir wired in configs) to unlock delegate flows. Added DigitalOcean MCP with inline token plus Z.AI Web Search (https://api.z.ai/api/mcp/web_search_prime/mcp) and Vision (npx -y @z_ai/mcp-server) servers across Codex CLI (~/.codex/config.toml), Claude Code (claude-code/.claude/.claude.json), and Claude Z.AI (claude-zai/.claude/.claude.json); verified connectivity via codex mcp list and claude mcp list. Exercised delegates and documented outcomes; noted the Web Search transport limitation for follow-up. Updated runbooks (DigitalOcean Operations Guide[^6], .docs/agents/digitalocean-operations.md) and the infrastructure expansion roadmap to document the new MCP surface area; tokens noted for rotation. 2025-11-02 follow-up: removed the Z.AI Web Search HTTP entry from Codex CLI config to prevent missing field command errors until native HTTP transports land; catalog + AGENTS docs updated with the mitigation.
