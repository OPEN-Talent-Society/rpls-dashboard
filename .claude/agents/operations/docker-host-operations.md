...
> Related: DevOps Operations Guide[^1], Docmost & NocoDB Backup SOP[^4], Migration Activity Log[^8], Docmost & NocoDB Migration Programme[^7].
...
- Upgrades: `sudo docker compose pull && sudo docker compose up -d`​; log in Migration Activity Log[^8].
...
- Record every significant change in Migration Activity Log[^8] and trackers like Docmost & NocoDB Project Tracker[^10] with:
...
[^1]: # devops-operations
    ...
    # DevOps Operations Guide
     ...
     Related playbooks: DigitalOcean Operations Guide[^2], [Docker Host Operations Guide](#20251103062240-w2ss5dy), Cortex (SiYuan) Operations Agent[^5], Docmost &amp; NocoDB Migration Programme[^7].

    #devops #ops #automation #cicd

    ## Core Responsibilities

     Own CI/CD, configuration management, and observability for all self-hosted services. Enforce infrastructure-as-code (Terraform or Ansible) for OCI, DigitalOcean, and future clouds. Maintain environment parity (dev → staging → prod) and document release cadences in Migration Activity Log[^8].
...
[^2]: # digitalocean-operations
    ...
    # DigitalOcean Operations Guide
     ...
     Related playbooks: DevOps Operations Guide[^1], [Docker Host Operations Guide](#20251103062240-w2ss5dy), Network Engineering Playbook[^3], Docmost &amp; NocoDB Backup SOP[^4].
    ...
    ## Networking
     ...
     VPC: isolate workloads per environment; record CIDRs in Network Engineering Playbook[^3]. Floating IPs: attach for HA; update DNS accordingly. Monitoring: enable DO metrics agent; integrate with Uptime Kuma.
...
[^3]: # network-engineering-playbook
...
[^5]: # cortex-siyuan-ops
    ...
    # Cortex (SiYuan) Operations Agent
     ...
     Start with Cortex (SiYuan) Operations &amp; Usage Guide[^6] and Migration Activity Log[^8] for system context.
    ...
    ## Runbook Links
     ...
     Cortex (SiYuan) Operations &amp; Usage Guide[^6] Docmost &amp; NocoDB Migration Programme[^7] scripts/sync/siyuan-export.py


[^6]: # cortex-siyuan-system
    ...
    # Cortex (SiYuan) Operations & Usage Guide
     ...
     Linked resources: Cortex (SiYuan) Operations Agent[^5], Docmost &amp; NocoDB Migration Programme[^7], Migration Activity Log[^8]
...
[^8]: # migration-tasks
    ...
    ## 2025-10-22 – Dashboard IA Draft
     ...
     Captured the high-level layout, card structure, and data sources for dash.aienablement.academy within Docmost &amp; NocoDB Migration Programme[^7] under “Dashboard Information Architecture (Current Concept)” to guide the next UI iteration.
    ...
    ## 2025-11-04 – Cortex Agent Enablement
     ...
     Authored Cortex (SiYuan) Operations Agent[^5] and associated skills (cortex-task-log, cortex-notebook-curation). Extended mcp/cortex server with document creation, append, attribute update, and SQL query tools. Scaffolded plugins/cortex-ops with mirrored agent/skill docs and logging commands; published plugin manifest for Claude/Codex installs. Refreshed Cortex (SiYuan) Operations &amp; Usage Guide[^6] with MCP tool guidance and synced plugins via scripts/sync/claude-sync.sh. Verified MCP toolchain end-to-end via local stub test (create doc → append block → set attrs → SQL query). Added launcher script scripts/mcp/cortex-mcp.sh and registered the server in Claude configs; secrets load from ~/.config/cortex-mcp.env to expose the tools to Claude clients.

    ## 2025-11-03 – Cortex Governance Upgrade

    - Published Cortex Metadata Standards[^11] and Tag &amp; Hashtag Taxonomy[^12] to codify second-brain metadata and tagging.
    - Added dashboards: Projects Dashboard[^14], Knowledge Freshness Dashboard[^15], Automation Inventory Dashboard[^16].
    - Created quick-start template Metadata &amp; Tag Check Template[^17] and governance section in Knowledge Base Overview[^18].
...
[^9]: # docmost-nocodb-lessons
    ...
    ## Related Knowledge
     ...
     See Docmost &amp; NocoDB Migration Programme[^7] for the end-to-end project blueprint. Cross-check backup details in Docmost &amp; NocoDB Backup SOP[^4]. Reference field diary entries in Migration Activity Log[^8].

    #docmost #nocodb #postmortem #ops


[^10]: # docmost-nocodb-tracker
...
[^11]: # metadata-standards
    ...
    Document updates to this standard in Migration Activity Log[^8] and notify stewards in Docmost.
...
[^12]: # tag-taxonomy
    ...
    |​`#ops/automation`|MCP, scripts, n8n flows|[[Tools & Scripts Overview]], cortex-task-log[^13]|
    ...
    Update taxonomy changes in Migration Activity Log[^8] and ensure runbooks adopt the new tags during their next review.
...
[^13]: # cortex-task-log
    ...
    # Cortex Task Log Skill
     ...
     Operated by Cortex (SiYuan) Operations Agent[^5] and referenced in Cortex (SiYuan) Operations &amp; Usage Guide[^6].
...
[^15]: # knowledge-freshness
    ...
    Use this dashboard during weekly reviews to assign refresh tasks and log updates in Migration Activity Log[^8].


[^16]: # automation-inventory
...
[^17]: # metadata-check-template
    ...
    - [ ] Hashtags added in first paragraph using taxonomy from Tag &amp; Hashtag Taxonomy[^12].
...
[^18]: # knowledge-base-overview
    ...
    ## Core Playbooks
     ...
     Network Engineering Playbook[^3] – Subnetting, DNS, TLS, and connectivity guardrails. System Architecture Guidelines[^19] – Platform principles and reference architecture. SQL &amp; PostgreSQL Playbook[^20] – Database administration, tuning, and recovery guidance. Docmost &amp; NocoDB Lessons Learned[^9] – Field notes and caveats from the migration programme. Cortex (SiYuan) Operations &amp; Usage Guide[^6] – How the second brain is structured and automated.

    ## Projects & Programmes

     Docmost &amp; NocoDB Migration Programme[^7] – End-to-end plan for the documentation/data platform transition. Docmost &amp; NocoDB Backup SOP[^4] – Nightly/weekly/monthly backup procedures and scripts. Docmost &amp; NocoDB Project Tracker[^10] – Thin-slice progress and follow-ups. Infrastructure &amp; Operations Inventory[^21] – Estate map across OCI, Proxmox, DigitalOcean, SaaS. Cloudflare Workers MCP Architecture[^22] – Edge automation strategy and fallback design. Multi-Environment Infrastructure Operations Expansion[^23] – Roadmap for unified ops coverage. Projects Dashboard[^14] – Active initiative tracker. Knowledge Freshness Dashboard[^15] – Review cadence board. Automation Inventory Dashboard[^16] – Script and MCP coverage summary.

    ## Operational Logs

     Migration Activity Log[^8] – Chronological record of stack changes, incidents, and validations.
    ...
    - Cortex Metadata Standards[^11] – Required front matter, attributes, and review cadence.
    - Tag &amp; Hashtag Taxonomy[^12] – Controlled hashtags and domain namespaces.
    - Metadata &amp; Tag Check Template[^17] – Quick template for new notes.
...
[^25]: # oci-operations
    ...
    - Snapshot instance shape/boot volume ocids after major changes; note in Migration Activity Log[^8].
    ...
    - Tracker: Docmost &amp; NocoDB Project Tracker[^10].
    - Logbook: Migration Activity Log[^8].
...
[^26]: # tailscale-operations
    ...
    - Verify: `tailscale status --json | jq '.Self.TailscaleIPs'`​; log IP + tag in Migration Activity Log[^8].
