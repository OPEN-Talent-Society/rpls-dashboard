...
name: docmost-admin
description: Docmost collaboration platform specialist for administration, user management, content governance, and system optimization
model: opus
color: blue
id: docmost-admin
summary: Operational playbook for managing the Docmost wiki stack, user lifecycle, and infrastructure.
status: active
owner: ops
...
# Docmost Administration Guide

 Related: Docmost & NocoDB Migration Programme[^1], Docmost & NocoDB Backup SOP[^2], Cortex (SiYuan) Operations Agent[^3].

#docmost #documentation #ops

## Overview

- Deployment: `/srv/docmost` (Docker Compose with Postgres + Redis + Docmost app).
- Public URL: `https://wiki.aienablement.academy` (Caddy reverse proxy).
- Database: Postgres 16; backups stored via nightly `pg_dump`.

## Configuration

```
APP_URL=https://wiki.aienablement.academy
DATABASE_URL=postgres://docmost:***@docmost-postgres:5432/docmost
...
- Session/secret keys live in `/srv/docmost/.env` (0600); rotate quarterly and refresh the container.
- Attachments: map `/srv/docmost/storage` volume; ensure backups include uploads.
...
```
 ...
 Health check: `curl -sf https://wiki.aienablement.academy/api/health`​. User management: promote admins via UI or direct SQL (`UPDATE users SET role='admin' WHERE email='...'`​). Content governance: organize spaces, enforce templates. Backups: nightly DB dump + storage tar; test restore quarterly (see Docmost & NocoDB Backup SOP[^2]).

## Security

- SSO roadmap: Docmost supports OIDC/SAML; plan integration once IdP ready.
- Limit upload size in `.env` if needed.
- Monitor audit logs in `/srv/docmost/storage/audit.log` (if enabled).

## Troubleshooting

- Login issues: verify SMTP; clear `sessions` table if stale cookies cause errors.
- TLS errors: confirm Caddy config redirect from `docs.*`​ to `wiki.*`.
- Queue/backups: check `docmost-docmost-app-1` logs and Postgres connectivity.

## References

- Official repo – https://github.com/docmost/docmost
- Deployment README – https://github.com/docmost/docmost/blob/main/apps/server/README.md
  }

[^1]: # docmost-nocodb-plan
...
[^3]: # cortex-siyuan-ops
    ...
    ## Runbook Links
     ...
     Cortex (SiYuan) Operations &amp; Usage Guide[^4] Docmost &amp; NocoDB Migration Programme[^1] scripts/sync/siyuan-export.py


[^4]: # cortex-siyuan-system
    ...
    # Cortex (SiYuan) Operations & Usage Guide
     ...
     Linked resources: Cortex (SiYuan) Operations Agent[^3], Docmost &amp; NocoDB Migration Programme[^1], Migration Activity Log[^5]
...
[^5]: # migration-tasks
    ...
    ## 2025-10-22 – Dashboard IA Draft
     ...
     Captured the high-level layout, card structure, and data sources for dash.aienablement.academy within Docmost &amp; NocoDB Migration Programme[^1] under “Dashboard Information Architecture (Current Concept)” to guide the next UI iteration.
    ...
    ## 2025-11-04 – Cortex Agent Enablement
     ...
     Authored Cortex (SiYuan) Operations Agent[^3] and associated skills (cortex-task-log, cortex-notebook-curation). Extended mcp/cortex server with document creation, append, attribute update, and SQL query tools. Scaffolded plugins/cortex-ops with mirrored agent/skill docs and logging commands; published plugin manifest for Claude/Codex installs. Refreshed Cortex (SiYuan) Operations &amp; Usage Guide[^4] with MCP tool guidance and synced plugins via scripts/sync/claude-sync.sh. Verified MCP toolchain end-to-end via local stub test (create doc → append block → set attrs → SQL query). Added launcher script scripts/mcp/cortex-mcp.sh and registered the server in Claude configs; secrets load from ~/.config/cortex-mcp.env to expose the tools to Claude clients.
...
[^6]: # docmost-nocodb-lessons
    ...
    ## Related Knowledge
     ...
     See Docmost &amp; NocoDB Migration Programme[^1] for the end-to-end project blueprint. Cross-check backup details in Docmost &amp; NocoDB Backup SOP[^2]. Reference field diary entries in Migration Activity Log[^5].
...
[^8]: # digitalocean-operations
    ...
    # DigitalOcean Operations Guide
     ...
     Related playbooks: DevOps Operations Guide[^9], Docker Host Operations Guide[^10], Network Engineering Playbook[^11], Docmost &amp; NocoDB Backup SOP[^2].
...
[^9]: # devops-operations
    ...
    # DevOps Operations Guide
     ...
     Related playbooks: DigitalOcean Operations Guide[^8], Docker Host Operations Guide[^10], Cortex (SiYuan) Operations Agent[^3], Docmost &amp; NocoDB Migration Programme[^1].
...
[^10]: # docker-host-operations
    ...
    > Related: DevOps Operations Guide[^9], Docmost &amp; NocoDB Backup SOP[^2], Migration Activity Log[^5], Docmost &amp; NocoDB Migration Programme[^1].
    >
    ...
    - Backups: leverage scripts in `ops/backup/`​ (see Docmost &amp; NocoDB Backup SOP[^2]) and confirm nightly timers succeed.
    ...
    - Record every significant change in Migration Activity Log[^5] and trackers like Docmost &amp; NocoDB Project Tracker[^7] with:
...
[^14]: # cortex-task-log
    ...
    # Cortex Task Log Skill
     ...
     Operated by Cortex (SiYuan) Operations Agent[^3] and referenced in Cortex (SiYuan) Operations &amp; Usage Guide[^4].
...
[^19]: # knowledge-base-overview
    ...
    ## Core Playbooks
     ...
     Network Engineering Playbook[^11] – Subnetting, DNS, TLS, and connectivity guardrails. System Architecture Guidelines[^20] – Platform principles and reference architecture. SQL &amp; PostgreSQL Playbook[^21] – Database administration, tuning, and recovery guidance. Docmost &amp; NocoDB Lessons Learned[^6] – Field notes and caveats from the migration programme. Cortex (SiYuan) Operations &amp; Usage Guide[^4] – How the second brain is structured and automated.

    ## Projects & Programmes

     Docmost &amp; NocoDB Migration Programme[^1] – End-to-end plan for the documentation/data platform transition. Docmost &amp; NocoDB Backup SOP[^2] – Nightly/weekly/monthly backup procedures and scripts. Docmost &amp; NocoDB Project Tracker[^7] – Thin-slice progress and follow-ups. Infrastructure &amp; Operations Inventory[^22] – Estate map across OCI, Proxmox, DigitalOcean, SaaS. Cloudflare Workers MCP Architecture[^23] – Edge automation strategy and fallback design. Multi-Environment Infrastructure Operations Expansion[^24] – Roadmap for unified ops coverage. Projects Dashboard[^15] – Active initiative tracker. Knowledge Freshness Dashboard[^16] – Review cadence board. Automation Inventory Dashboard[^17] – Script and MCP coverage summary.
...
[^28]: # uptime-kuma-operations
    ...
    - Backup: export config via UI; include volume in Docmost &amp; NocoDB Backup SOP[^2].
...
[^29]: # formbricks-operations
    ...
    - Include `formbricks_db_data`​ in nightly backup (see Docmost &amp; NocoDB Backup SOP[^2]).
