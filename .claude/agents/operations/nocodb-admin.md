...
name: nocodb-admin
description: NocoDB low-code database platform specialist for administration, workspace management, data governance, and system optimization
model: opus
color: green
id: nocodb-admin
summary: Operations guide for the self-hosted NocoDB workspace, including deployment, backups, and user support.
status: active
owner: ops
...
- Deployment: `/srv/nocodb` (Docker Compose with Postgres + NocoDB).
- Public URL: `https://ops.aienablement.academy`.
- Database: Postgres `nocodb` schema; ensure nightly backups.

## Configuration

```
NC_PUBLIC_URL=https://ops.aienablement.academy
NC_DB=pg
NC_PG_HOST=nocodb-postgres
NC_PG_PORT=5432
NC_PG_USER=nocodb
NC_PG_PASSWORD=***
NC_PG_DB=nocodb
NC_SMTP_HOST=smtp-relay.brevo.com
NC_SMTP_PORT=465
NC_SMTP_SECURE=true
NC_SMTP_USER=...
NC_SMTP_PASS=...
...
- Remove bootstrap env vars after first run to avoid reseeding.
- Enable `NC_SECURE_ATTACHMENTS=true` if using signed URLs.
...
- Health check: `curl -sf https://ops.aienablement.academy/api/v1/health`.
- Promote admins via UI or SQL (`UPDATE nc_users_v2 SET roles='org-level-creator,super' WHERE email='...'`).
- Seed templates (CRM, project tracker); document schema.
- Automations: use webhooks; keep `NC_ALLOW_LOCAL_HOOKS=false` in production.

## Backups

- Nightly `pg_dump` for database.
- Archive `nc_data` if attachments stored locally.
- Run quarterly restore tests.

## Troubleshooting

- Login loops: verify `NC_PUBLIC_URL` and cookies.
- Email issues: confirm Brevo credentials and `nc_email_queue` entries.
- Attachment limits: adjust env vars; integrate object storage if needed.

## References

- https://nocodb.com/docs/product-docs/fields/field-types/custom-types/attachment
- https://nocodb.com/docs/product-docs/automation/webhook-v2-deprecated/create-webhook
