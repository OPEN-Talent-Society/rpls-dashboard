...
color: purple
id: n8n-operations
summary: Operations checklist for the n8n automation stack, including deployments, secrets, and flow management.
status: active
owner: ops
...
#automation #ops/runbook #class/runbook

## Overview

- Stack: `/srv/n8n` compose (n8n + Postgres).
- Proxy: Caddy serves `https://n8n.aienablement.academy`.
- Auth: Caddy basic auth + in-app users; disable telemetry for privacy.
...
- Persist `/home/node/.n8n`​ via `n8n_data` volume.
- Keep `N8N_ENCRYPTION_KEY` safe (required for credential restore).
...
- Health: `curl -sf https://n8n.aienablement.academy/healthz`​; monitor `/metrics`.
- Backups: nightly `pg_dump`​ of `n8n_db`​; include in Docmost & NocoDB Backup SOP[^1].
- User invites: via SMTP (Brevo credentials in `.env`).
- Adjust concurrency (`N8N_CONCURRENCY_PRODUCTION_LIMIT`) as CPU availability allows.

## Troubleshooting

- Workflow errors: `docker compose logs -f n8n-app`.
- Queue problems: ensure Redis/queue config when scaling horizontally.
- Email failures: check Brevo logs; confirm TLS config (`N8N_SMTP_SSL=true`, port 465).
- Performance: enable Prometheus metrics; integrate with Uptime Kuma dashboard.

## References

- Monitoring: https://github.com/n8n-io/n8n-docs/blob/main/docs/hosting/logging-monitoring/monitoring.md
- User management SMTP: https://github.com/n8n-io/n8n-docs/blob/main/docs/hosting/configuration/user-management-self-hosted.md
