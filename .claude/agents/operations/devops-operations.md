...
# DevOps Operations Guide
 ...
 Related playbooks: DigitalOcean Operations Guide[^1], Docker Host Operations Guide[^2], Cortex (SiYuan) Operations Agent[^8], Docmost & NocoDB Migration Programme[^6].

#devops #ops #automation #cicd

## Core Responsibilities

 Own CI/CD, configuration management, and observability for all self-hosted services. Enforce infrastructure-as-code (Terraform or Ansible) for OCI, DigitalOcean, and future clouds. Maintain environment parity (dev → staging → prod) and document release cadences in Migration Activity Log[^4].
...
[^1]: # digitalocean-operations
    ...
    # DigitalOcean Operations Guide
     ...
     Related playbooks: [DevOps Operations Guide](#20251103060350-fbfwfhq), Docker Host Operations Guide[^2], Network Engineering Playbook[^18], Docmost &amp; NocoDB Backup SOP[^3].
    ...
    ## Networking
     ...
     VPC: isolate workloads per environment; record CIDRs in Network Engineering Playbook[^18]. Floating IPs: attach for HA; update DNS accordingly. Monitoring: enable DO metrics agent; integrate with Uptime Kuma.
...
[^2]: # docker-host-operations
    ...
    > Related: [DevOps Operations Guide](#20251103060350-fbfwfhq), Docmost &amp; NocoDB Backup SOP[^3], Migration Activity Log[^4], Docmost &amp; NocoDB Migration Programme[^6].
    >
    ...
    - Upgrades: `sudo docker compose pull && sudo docker compose up -d`​; log in Migration Activity Log[^4].
    - Backups: leverage scripts in `ops/backup/`​ (see Docmost &amp; NocoDB Backup SOP[^3]) and confirm nightly timers succeed.
    ...
    - Record every significant change in Migration Activity Log[^4] and trackers like Docmost &amp; NocoDB Project Tracker[^7] with:
...
[^4]: # migration-tasks
    ...
    ## 2025-10-22 – Dashboard IA Draft
     ...
     Captured the high-level layout, card structure, and data sources for dash.aienablement.academy within Docmost &amp; NocoDB Migration Programme[^6] under “Dashboard Information Architecture (Current Concept)” to guide the next UI iteration.
    ...
    ## 2025-11-04 – Cortex Agent Enablement
     ...
     Authored Cortex (SiYuan) Operations Agent[^8] and associated skills (cortex-task-log, cortex-notebook-curation). Extended mcp/cortex server with document creation, append, attribute update, and SQL query tools. Scaffolded plugins/cortex-ops with mirrored agent/skill docs and logging commands; published plugin manifest for Claude/Codex installs. Refreshed Cortex (SiYuan) Operations &amp; Usage Guide[^9] with MCP tool guidance and synced plugins via scripts/sync/claude-sync.sh. Verified MCP toolchain end-to-end via local stub test (create doc → append block → set attrs → SQL query). Added launcher script scripts/mcp/cortex-mcp.sh and registered the server in Claude configs; secrets load from ~/.config/cortex-mcp.env to expose the tools to Claude clients.

    ## 2025-11-03 – Cortex Governance Upgrade

    - Published Cortex Metadata Standards[^10] and Tag &amp; Hashtag Taxonomy[^11] to codify second-brain metadata and tagging.
    - Added dashboards: Projects Dashboard[^13], Knowledge Freshness Dashboard[^14], Automation Inventory Dashboard[^15].
    - Created quick-start template Metadata &amp; Tag Check Template[^16] and governance section in Knowledge Base Overview[^17].
    - Backfilled block references across runbooks, backups, and automation logs to enable graph navigation.
    - Imported key infrastructure runbooks linux-server-operations[^24], oci-operations[^25], tailscale-operations[^26], uptime-kuma-operations[^27], formbricks-operations[^28] into Cortex and wired them into Agents Overview.


[^5]: # docmost-nocodb-lessons
    ...
    ## Related Knowledge
     ...
     See Docmost &amp; NocoDB Migration Programme[^6] for the end-to-end project blueprint. Cross-check backup details in Docmost &amp; NocoDB Backup SOP[^3]. Reference field diary entries in Migration Activity Log[^4].

    #docmost #nocodb #postmortem #ops


[^6]: # docmost-nocodb-plan
...
[^8]: # cortex-siyuan-ops
    ...
    # Cortex (SiYuan) Operations Agent
     ...
     Start with Cortex (SiYuan) Operations &amp; Usage Guide[^9] and Migration Activity Log[^4] for system context.
    ...
    ## Runbook Links
     ...
     Cortex (SiYuan) Operations &amp; Usage Guide[^9] Docmost &amp; NocoDB Migration Programme[^6] scripts/sync/siyuan-export.py


[^9]: # cortex-siyuan-system
    ...
    # Cortex (SiYuan) Operations & Usage Guide
     ...
     Linked resources: Cortex (SiYuan) Operations Agent[^8], Docmost &amp; NocoDB Migration Programme[^6], Migration Activity Log[^4]
...
[^10]: # metadata-standards
    ...
    Document updates to this standard in Migration Activity Log[^4] and notify stewards in Docmost.
...
[^11]: # tag-taxonomy
    ...
    |​`#ops/automation`|MCP, scripts, n8n flows|[[Tools & Scripts Overview]], cortex-task-log[^12]|
    ...
    Update taxonomy changes in Migration Activity Log[^4] and ensure runbooks adopt the new tags during their next review.
...
[^12]: # cortex-task-log
    ...
    # Cortex Task Log Skill
     ...
     Operated by Cortex (SiYuan) Operations Agent[^8] and referenced in Cortex (SiYuan) Operations &amp; Usage Guide[^9].
...
[^14]: # knowledge-freshness
    ...
    Use this dashboard during weekly reviews to assign refresh tasks and log updates in Migration Activity Log[^4].


[^15]: # automation-inventory
...
[^16]: # metadata-check-template
    ...
    - [ ] Hashtags added in first paragraph using taxonomy from Tag &amp; Hashtag Taxonomy[^11].
...
[^17]: # knowledge-base-overview
    ...
    ## Core Playbooks
     ...
     Network Engineering Playbook[^18] – Subnetting, DNS, TLS, and connectivity guardrails. System Architecture Guidelines[^19] – Platform principles and reference architecture. SQL &amp; PostgreSQL Playbook[^20] – Database administration, tuning, and recovery guidance. Docmost &amp; NocoDB Lessons Learned[^5] – Field notes and caveats from the migration programme. Cortex (SiYuan) Operations &amp; Usage Guide[^9] – How the second brain is structured and automated.

    ## Projects & Programmes

     Docmost &amp; NocoDB Migration Programme[^6] – End-to-end plan for the documentation/data platform transition. Docmost &amp; NocoDB Backup SOP[^3] – Nightly/weekly/monthly backup procedures and scripts. Docmost &amp; NocoDB Project Tracker[^7] – Thin-slice progress and follow-ups. Infrastructure &amp; Operations Inventory[^21] – Estate map across OCI, Proxmox, DigitalOcean, SaaS. Cloudflare Workers MCP Architecture[^22] – Edge automation strategy and fallback design. Multi-Environment Infrastructure Operations Expansion[^23] – Roadmap for unified ops coverage. Projects Dashboard[^13] – Active initiative tracker. Knowledge Freshness Dashboard[^14] – Review cadence board. Automation Inventory Dashboard[^15] – Script and MCP coverage summary.

    ## Operational Logs

     Migration Activity Log[^4] – Chronological record of stack changes, incidents, and validations.
    ...
    - Cortex Metadata Standards[^10] – Required front matter, attributes, and review cadence.
    - Tag &amp; Hashtag Taxonomy[^11] – Controlled hashtags and domain namespaces.
    - Metadata &amp; Tag Check Template[^16] – Quick template for new notes.


[^18]: # network-engineering-playbook
...
[^19]: # system-architecture-guidelines
...
[^22]: # cloudflare-workers-mcp-architecture
...
[^23]: # infrastructure-ops-expansion
...
[^25]: # oci-operations
    ...
    - Snapshot instance shape/boot volume ocids after major changes; note in Migration Activity Log[^4].
    ...
    - Tracker: Docmost &amp; NocoDB Project Tracker[^7].
    - Logbook: Migration Activity Log[^4].


[^26]: # tailscale-operations
    ...
    - Verify: `tailscale status --json | jq '.Self.TailscaleIPs'`​; log IP + tag in Migration Activity Log[^4].
...
[^27]: # uptime-kuma-operations
    ...
    - Backup: export config via UI; include volume in Docmost &amp; NocoDB Backup SOP[^3].
...
[^28]: # formbricks-operations
    ...
    - Include `formbricks_db_data`​ in nightly backup (see Docmost &amp; NocoDB Backup SOP[^3]).
