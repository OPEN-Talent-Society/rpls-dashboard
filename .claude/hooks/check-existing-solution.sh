...
PATTERNS=$(/opt/homebrew/bin/claude-flow memory search \
  --namespace "patterns" \
...
ERRORS=$(/opt/homebrew/bin/claude-flow memory search \
  --namespace "errors" \
  --pattern "errors/*" \
  --limit 3 2>/dev/null)
...
LEARNINGS=$(/opt/homebrew/bin/claude-flow memory search \
  --namespace "learnings" \
  --pattern "learnings/*" \
  --limit 3 2>/dev/null)
