...
pnpm dlx claude-flow init --hooks
```
...
pnpm dlx claude-flow hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
pnpm dlx claude-flow hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
pnpm dlx claude-flow hook session-end --session-id "dev-session" --export-metrics
```
...
Examples:
  pnpm dlx claude-flow hook pre-edit --file "src/auth/login.js"
  pnpm dlx claude-flow hook pre-edit -f "config/db.js" --validate-syntax
  pnpm dlx claude-flow hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
pnpm dlx claude-flow hook pre-bash --command <cmd>
...
Examples:
  pnpm dlx claude-flow hook pre-bash -c "rm -rf /tmp/cache"
  pnpm dlx claude-flow hook pre-bash --command "docker build ." --estimate-resources
```
...
Examples:
  pnpm dlx claude-flow hook pre-task --description "Implement user authentication"
  pnpm dlx claude-flow hook pre-task -d "Continue API dev" --load-memory
  pnpm dlx claude-flow hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
pnpm dlx claude-flow hook pre-search --query <query>
...
Examples:
  pnpm dlx claude-flow hook pre-search -q "authentication middleware"
```
...
Examples:
  pnpm dlx claude-flow hook post-edit --file "src/components/Button.jsx"
  pnpm dlx claude-flow hook post-edit -f "api/auth.js" --memory-key "auth/login"
  pnpm dlx claude-flow hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
pnpm dlx claude-flow hook post-bash --command <cmd>
...
Examples:
  pnpm dlx claude-flow hook post-bash -c "ppnpm test" --update-metrics
```
...
Examples:
  pnpm dlx claude-flow hook post-task --task-id "auth-implementation"
  pnpm dlx claude-flow hook post-task -t "api-refactor" --analyze-performance
  pnpm dlx claude-flow hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
pnpm dlx claude-flow hook post-search --query <query> --results <path>
...
Examples:
  pnpm dlx claude-flow hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
pnpm dlx claude-flow hook mcp-initialized --swarm-id <id>
...
```bash
pnpm dlx claude-flow hook agent-spawned --agent-id <id> --type <type>
...
```bash
pnpm dlx claude-flow hook task-orchestrated --task-id <id>
...
```bash
pnpm dlx claude-flow hook neural-trained --pattern <name>
...
```bash
pnpm dlx claude-flow hook memory-sync --namespace <ns>
...
```bash
pnpm dlx claude-flow hook session-start --session-id <id>
...
```bash
pnpm dlx claude-flow hook session-restore --session-id <id>
...
Examples:
  pnpm dlx claude-flow hook session-restore --session-id "swarm-20241019"
  pnpm dlx claude-flow hook session-restore -s "feature-auth" --restore-memory
```
...
Examples:
  pnpm dlx claude-flow hook session-end --session-id "dev-session-2024"
  pnpm dlx claude-flow hook session-end -s "feature-auth" --export-metrics --generate-summary
  pnpm dlx claude-flow hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
pnpm dlx claude-flow hook notify --message <msg>
...
Examples:
  pnpm dlx claude-flow hook notify -m "Task completed" --level info
  pnpm dlx claude-flow hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "pnpm dlx claude-flow hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "pnpm dlx claude-flow hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "pnpm dlx claude-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "pnpm dlx claude-flow hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm dlx claude-flow hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
pnpm dlx claude-flow hook pre-task --description "Build REST API"
...
```javascript
// Hook command
pnpm dlx claude-flow hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
pnpm dlx claude-flow hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  pnpm dlx claude-flow hook pre-edit --file "$FILE" --validate-syntax
  ...
  pnpm dlx claude-flow hook post-edit --file "$FILE" --auto-format
done
...
pnpm dlx claude-flow hook notify \
...
TRUTH_SCORE=$(pnpm dlx claude-flow metrics score --format json | jq -r '.truth_score')
...
pnpm dlx claude-flow hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
pnpm dlx claude-flow hook session-restore \
...
pnpm dlx claude-flow hook session-start --session-id "fullstack-feature"
...
# Backend work
pnpm dlx claude-flow hook pre-edit --file "api/profile.js"
# ... implement backend ...
pnpm dlx claude-flow hook post-edit \
  --file "api/profile.js" \
...
pnpm dlx claude-flow hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
pnpm dlx claude-flow hook post-edit \
...
pnpm dlx claude-flow hook session-start --session-id "debug-memory-leak"
...
pnpm dlx claude-flow hook pre-search --query "EventEmitter"
# ... search executes ...
pnpm dlx claude-flow hook post-search \
  --query "EventEmitter" \
  --cache-results
...
pnpm dlx claude-flow hook pre-task --description "Analyze code complexity"
# ... analysis ...
pnpm dlx claude-flow hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  pnpm dlx claude-flow hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  pnpm dlx claude-flow hook post-edit \
    --file "$file" \
...
pnpm dlx claude-flow hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .claude-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
pnpm dlx claude-flow hook validate-config
```
...
- `pnpm dlx claude-flow init --hooks` - Initialize hooks system
- `pnpm dlx claude-flow hook --list` - List available hooks
- `pnpm dlx claude-flow hook --test <hook>` - Test specific hook
- `pnpm dlx claude-flow memory usage` - Manage memory
- `pnpm dlx claude-flow agent spawn` - Spawn agents
- `pnpm dlx claude-flow swarm init` - Initialize swarm
