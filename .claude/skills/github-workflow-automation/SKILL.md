...
pnpm dlx ruv-swarm actions generate-workflow \
  --analyze-codebase \
  --detect-languages \
...
pnpm dlx ruv-swarm actions optimize \
  --workflow ".github/workflows/ci.yml" \
  --suggest-parallelization

# Analyze failed runs
gh run view <run-id> --json jobs,conclusion | \
  pnpm dlx ruv-swarm actions analyze-failure \
    --suggest-fixes
```
...
```bash
# Usage example
/opt/homebrew/bin/agentic-flow github gh-coordinator \
...
gh pr create --title "Feature: New capability" \
  ...
  pnpm dlx ruv-swarm actions pr-validate \
...
/opt/homebrew/bin/agentic-flow github issue-tracker \
...
/opt/homebrew/bin/agentic-flow github release-manager \
...
/opt/homebrew/bin/agentic-flow github repo-architect \
...
/opt/homebrew/bin/agentic-flow github ci-orchestrator \
...
```bash
# Security audit
pnpm dlx ruv-swarm actions security \
  --deep-scan \
  --compliance-check \
  --create-issues
```
  ...
  swarm-analysis:
    ...
    steps:
      ...
      - name: Analyze Changes
        run: |
          pnpm dlx ruv-swarm actions analyze \
            --commit ${{ github.sha }} \
            --suggest-tests \
            --optimize-pipeline
```
  ...
  detect-and-build:
    ...
    steps:
      ...
      - name: Detect Languages
        id: detect
        run: |
          pnpm dlx ruv-swarm actions detect-stack \
            --output json > stack.json

      - name: Dynamic Build Matrix
        run: |
          pnpm dlx ruv-swarm actions create-matrix \
            --from stack.json \
            --parallel-builds
```
  ...
  security-swarm:
    ...
    steps:
      - name: Security Analysis Swarm
        run: |
          SECURITY_ISSUES=$(pnpm dlx ruv-swarm actions security \
            --deep-scan \
            --format json)
  ...
  heal-pipeline:
    ...
    steps:
      - name: Diagnose and Fix
        run: |
          pnpm dlx ruv-swarm actions self-heal \
  ...
  progressive-deploy:
    ...
    steps:
      - name: Analyze Risk
        id: risk
        run: |
          pnpm dlx ruv-swarm actions deploy-risk \
            --changes ${{ github.sha }} \
            --history 30d

      - name: Choose Strategy
        run: |
          pnpm dlx ruv-swarm actions deploy-strategy \
  ...
  perf-swarm:
    ...
    steps:
      - name: Performance Analysis
        run: |
          pnpm dlx ruv-swarm actions perf-test \
            --baseline main \
            --threshold 10% \
    ...
    steps:
      - name: Multi-Agent Validation
        run: |
          ...
          RESULTS=$(pnpm dlx ruv-swarm actions pr-validate \
            --spawn-agents "linter,tester,security,docs" \
            --parallel \
            --pr-data "$PR_DATA")
...
jobs:
  release:
    ...
    steps:
      - name: Release Swarm
        run: |
          pnpm dlx ruv-swarm actions release \
            --analyze-changes \
            --generate-notes \
            --create-artifacts \
            --publish-smart
...
pnpm dlx ruv-swarm actions analytics \
  --workflow "ci.yml" \
  --period 30d \
...
pnpm dlx ruv-swarm actions cost-optimize \
  --analyze-usage \
  --suggest-caching \
...
pnpm dlx ruv-swarm actions failure-patterns \
  --period 90d \
  --classify-failures \
  --suggest-preventions
```
...
pnpm dlx ruv-swarm actions resources \
  --analyze-usage \
  --suggest-runners \
  --cost-optimize
```
...
- name: Swarm Test Selection
  run: |
    pnpm dlx ruv-swarm actions smart-test \
...
jobs:
  generate-matrix:
    ...
    steps:
      - id: set-matrix
        run: |
          MATRIX=$(pnpm dlx ruv-swarm actions test-matrix \
            --detect-frameworks \
            --optimize-coverage)
...
pnpm dlx ruv-swarm actions parallel-strategy \
...
pnpm dlx ruv-swarm actions predict \
  --analyze-history \
  --identify-risks \
  --suggest-preventive
```
...
pnpm dlx ruv-swarm actions recommend \
  --analyze-repo \
  --suggest-workflows \
...
pnpm dlx ruv-swarm actions auto-optimize \
  --monitor-performance \
  --apply-improvements \
  --track-savings
```
...
/opt/homebrew/bin/agentic-flow hooks pre-task \
  --description "PR review workflow" \
  --context "pr-123"

# During task: Track progress
/opt/homebrew/bin/agentic-flow hooks notify \
  --message "Completed security scan" \
  --type "github-action"

# Post-task: Export results
/opt/homebrew/bin/agentic-flow hooks post-task \
...
jobs:
  swarm-task:
    ...
    steps:
      - name: Initialize Swarm
        run: |
          pnpm dlx ruv-swarm init --topology ${{ inputs.topology }}
```
...
- name: Cache Swarm Dependencies
  ...
  with:
    path: ~/.pnpm-store
    key: ${{ runner.os }}-swarm-${{ hashFiles('**/pnpm-lock.yaml') }}
```
...
- name: Setup Swarm
  ...
  run: |
    pnpm dlx ruv-swarm init --config "$SWARM_CONFIG"
```
...
- name: Audit Swarm Actions
  run: |
    pnpm dlx ruv-swarm actions audit \
      --export-logs \
      --compliance-report
```
...
- name: Quick Fail Check
  run: |
    if ! pnpm dlx ruv-swarm actions pre-check; then
...
- name: Debug Swarm
  run: |
    pnpm dlx ruv-swarm actions debug \
      --verbose \
      --trace-agents \
      --export-logs
  env:
...
pnpm dlx ruv-swarm actions profile \
  --workflow "ci.yml" \
  --identify-slow-steps \
...
pnpm dlx ruv-swarm actions analyze-logs \
  --directory ./logs \
  --identify-errors
```
...
jobs:
  initialize:
    ...
    steps:
      - id: init
        run: |
          SWARM_ID=$(pnpm dlx ruv-swarm init --topology mesh --output json | jq -r '.id')
  ...
  backend:
    ...
    steps:
      ...
      - name: Backend Tests
        run: |
          pnpm dlx ruv-swarm agents spawn --type tester \
  ...
  security:
    ...
    steps:
      ...
      - name: Security Scan
        run: |
          pnpm dlx ruv-swarm agents spawn --type security \
            --task "Security audit" \
            --swarm-id ${{ needs.initialize.outputs.swarm-id }}
  ...
  deploy:
    ...
    steps:
      - name: Deploy
        run: |
          pnpm dlx ruv-swarm actions deploy \
...
jobs:
  detect-changes:
    ...
    steps:
      ...
      - id: detect
        run: |
          PACKAGES=$(pnpm dlx ruv-swarm actions detect-changes \
            --monorepo \
            --output json)
  ...
  build-packages:
    ...
    steps:
      - name: Build Package
        run: |
          pnpm dlx ruv-swarm actions build \
...
/opt/homebrew/bin/agentic-flow github sync-coordinator \
...
#### Workflow Generation
```bash
pnpm dlx ruv-swarm actions generate-workflow [options]
...
#### Optimization
```bash
pnpm dlx ruv-swarm actions optimize [options]
...
#### Analysis
```bash
pnpm dlx ruv-swarm actions analyze [options]
...
#### Testing
```bash
pnpm dlx ruv-swarm actions smart-test [options]
...
#### Security
```bash
pnpm dlx ruv-swarm actions security [options]
...
#### Deployment
```bash
pnpm dlx ruv-swarm actions deploy [options]
...
#### Monitoring
```bash
pnpm dlx ruv-swarm actions analytics [options]
