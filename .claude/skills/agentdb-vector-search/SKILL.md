...
pnpm dlx agentdb@latest init ./vectors.db

# Custom dimensions for different embedding models
pnpm dlx agentdb@latest init ./vectors.db --dimension 768  # sentence-transformers
pnpm dlx agentdb@latest init ./vectors.db --dimension 384  # all-MiniLM-L6-v2

# Use preset configurations
pnpm dlx agentdb@latest init ./vectors.db --preset small   # <10K vectors
pnpm dlx agentdb@latest init ./vectors.db --preset medium  # 10K-100K vectors
pnpm dlx agentdb@latest init ./vectors.db --preset large   # >100K vectors

# In-memory database for testing
pnpm dlx agentdb@latest init ./vectors.db --in-memory
```
...
pnpm dlx agentdb@latest query ./vectors.db "[0.1,0.2,0.3,...]"

# Top-k results
pnpm dlx agentdb@latest query ./vectors.db "[0.1,0.2,0.3]" -k 10

# With similarity threshold (cosine similarity)
pnpm dlx agentdb@latest query ./vectors.db "0.1 0.2 0.3" -t 0.75 -m cosine

# Different distance metrics
pnpm dlx agentdb@latest query ./vectors.db "[...]" -m euclidean  # L2 distance
pnpm dlx agentdb@latest query ./vectors.db "[...]" -m dot        # Dot product

# JSON output for automation
pnpm dlx agentdb@latest query ./vectors.db "[...]" -f json -k 5

# Verbose output with distances
pnpm dlx agentdb@latest query ./vectors.db "[...]" -v
```
...
pnpm dlx agentdb@latest export ./vectors.db ./backup.json

# Import vectors from JSON
pnpm dlx agentdb@latest import ./backup.json

# Get database statistics
pnpm dlx agentdb@latest stats ./vectors.db
```
...
pnpm dlx agentdb@latest mcp

# Add to Claude Code (one-time setup)
claude mcp add agentdb pnpm dlx agentdb@latest mcp
...
pnpm dlx agentdb@latest benchmark
...
pnpm dlx agentdb@latest query ./db.sqlite "[...]" -m cosine

# Euclidean distance (L2 norm)
pnpm dlx agentdb@latest query ./db.sqlite "[...]" -m euclidean

# Dot product (for normalized vectors)
pnpm dlx agentdb@latest query ./db.sqlite "[...]" -m dot
```
...
pnpm dlx agentdb@latest query ./db.sqlite "[...]" -t 0.8  # Higher threshold
...
pnpm dlx agentdb@latest init ./db.sqlite --dimension 768
```
...
- MCP Integration: `pnpm dlx agentdb@latest mcp` for Claude Code
- Website: https://agentdb.ruv.io
- CLI Help: `pnpm dlx agentdb@latest --help`
- Command Help: `pnpm dlx agentdb@latest help <command>`
