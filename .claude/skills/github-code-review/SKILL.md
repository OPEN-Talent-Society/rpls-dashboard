...
gh pr view 123 --json files,diff | pnpm dlx ruv-swarm github review-init --pr 123
...
pnpm dlx ruv-swarm github review-init \
  --pr 123 \
  --pr-data "$PR_DATA" \
  --diff "$PR_DIFF" \
...
SECURITY_RESULTS=$(pnpm dlx ruv-swarm github review-security \
  --pr 123 \
  --files "$CHANGED_FILES" \
...
pnpm dlx ruv-swarm github review-performance \
  --pr 123 \
...
```bash
# Architecture review
pnpm dlx ruv-swarm github review-architecture \
  --pr 123 \
...
pnpm dlx ruv-swarm github review-style \
  --pr 123 \
...
gh pr view 123 --json body,title,labels,files | pnpm dlx ruv-swarm swarm create-from-pr

# Auto-spawn agents based on PR labels
gh pr view 123 --json labels | pnpm dlx ruv-swarm swarm auto-spawn

# Create swarm with full PR context
gh pr view 123 --json body,labels,author,assignees | \
  pnpm dlx ruv-swarm swarm init --from-pr-data
```
...
pnpm dlx ruv-swarm github pr-topology --pr 123
```
...
createServer((req, res) => {
  if (req.url === '/github-webhook') {
    ...
    if (event.action === 'opened' && event.pull_request) {
      execSync(`pnpm dlx ruv-swarm github pr-init ${event.pull_request.number}`);
    }

    if (event.comment && event.comment.body.startsWith('/swarm')) {
      const command = event.comment.body;
      execSync(`pnpm dlx ruv-swarm github handle-comment --pr ${event.issue.number} --command "${command}"`);
    }
    ...
    steps:
      ...
      - name: Run Review Swarm
        run: |
          ...
          # Run swarm review
          REVIEW_OUTPUT=$(pnpm dlx ruv-swarm github review-all \
            --pr $PR_NUM \
            --pr-data "$PR_DATA" \
            --diff "$PR_DIFF" \
...
COMMENTS=$(pnpm dlx ruv-swarm github review-comment \
  --pr 123 \
  --diff "$PR_DIFF" \
  --files "$PR_FILES" \
  --style "constructive" \
...
pnpm dlx ruv-swarm github review-comments \
  --pr 123 \
...
pnpm dlx ruv-swarm github quality-gates \
  --define '{
...
pnpm dlx ruv-swarm github review-metrics \
  --period 30d \
...
pnpm dlx ruv-swarm github review-context \
  --pr 123 \
  --load-related-prs \
  --analyze-impact \
...
pnpm dlx ruv-swarm github review-learn \
...
pnpm dlx ruv-swarm github review-train \
  --learn-patterns \
  --adapt-to-style \
  --improve-accuracy
```
...
pnpm dlx ruv-swarm github review-batch \
  --prs "123,124,125" \
  --check-consistency \
  --verify-integration \
  --combined-impact
...
pnpm dlx ruv-swarm github multi-pr \
  --prs "123,124,125" \
  --strategy "parallel" \
  --share-memory
```
...
pnpm dlx ruv-swarm github register-agent \
  ...
  swarm-review:
    ...
    steps:
      - name: Run Swarm Review
        run: |
          pnpm dlx ruv-swarm github review-all \
...
pnpm dlx ruv-swarm github pr-fix 123 \
...
PROGRESS=$(pnpm dlx ruv-swarm github pr-progress 123 --format markdown)
...
gh pr view 456 --json body,labels,files | \
  pnpm dlx ruv-swarm github pr-init 456 \
...
pnpm dlx ruv-swarm github pr-init 789 \
  --topology mesh \
...
pnpm dlx ruv-swarm github review-dashboard \
  --real-time \
...
pnpm dlx ruv-swarm github review-report \
  --format "markdown" \
...
pnpm dlx ruv-swarm github pr-report 123 \
...
pnpm dlx ruv-swarm github export-metrics \
  --pr 123 \
  --to-insights \
  --dashboard-url
```
...
SWARM_STATUS=$(pnpm dlx ruv-swarm github pr-status 123)
...
```bash
# Check swarm status
pnpm dlx ruv-swarm swarm-status

# Verify GitHub CLI authentication
gh auth status

# Re-initialize swarm
pnpm dlx ruv-swarm github review-init --pr 123 --force
```
...
pnpm dlx ruv-swarm github review-comments --pr 123 --batch
```
...
pnpm dlx ruv-swarm github review-init --pr 123 --incremental

# Reduce agent count
pnpm dlx ruv-swarm github review-init --pr 123 --agents "security,style" --max-agents 3

# Enable parallel processing
pnpm dlx ruv-swarm github review-init --pr 123 --parallel --cache-results
```
