...
/opt/homebrew/bin/claude-flow init --hooks
```
...
/opt/homebrew/bin/claude-flow hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
/opt/homebrew/bin/claude-flow hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
/opt/homebrew/bin/claude-flow hook session-end --session-id "dev-session" --export-metrics
```
...
Examples:
  /opt/homebrew/bin/claude-flow hook pre-edit --file "src/auth/login.js"
  /opt/homebrew/bin/claude-flow hook pre-edit -f "config/db.js" --validate-syntax
  /opt/homebrew/bin/claude-flow hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
/opt/homebrew/bin/claude-flow hook pre-bash --command <cmd>
...
Examples:
  /opt/homebrew/bin/claude-flow hook pre-bash -c "rm -rf /tmp/cache"
  /opt/homebrew/bin/claude-flow hook pre-bash --command "docker build ." --estimate-resources
```
...
Examples:
  /opt/homebrew/bin/claude-flow hook pre-task --description "Implement user authentication"
  /opt/homebrew/bin/claude-flow hook pre-task -d "Continue API dev" --load-memory
  /opt/homebrew/bin/claude-flow hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
/opt/homebrew/bin/claude-flow hook pre-search --query <query>
...
Examples:
  /opt/homebrew/bin/claude-flow hook pre-search -q "authentication middleware"
```
...
Examples:
  /opt/homebrew/bin/claude-flow hook post-edit --file "src/components/Button.jsx"
  /opt/homebrew/bin/claude-flow hook post-edit -f "api/auth.js" --memory-key "auth/login"
  /opt/homebrew/bin/claude-flow hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
/opt/homebrew/bin/claude-flow hook post-bash --command <cmd>
...
Examples:
  /opt/homebrew/bin/claude-flow hook post-bash -c "ppnpm test" --update-metrics
```
...
Examples:
  /opt/homebrew/bin/claude-flow hook post-task --task-id "auth-implementation"
  /opt/homebrew/bin/claude-flow hook post-task -t "api-refactor" --analyze-performance
  /opt/homebrew/bin/claude-flow hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
/opt/homebrew/bin/claude-flow hook post-search --query <query> --results <path>
...
Examples:
  /opt/homebrew/bin/claude-flow hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
/opt/homebrew/bin/claude-flow hook mcp-initialized --swarm-id <id>
...
```bash
/opt/homebrew/bin/claude-flow hook agent-spawned --agent-id <id> --type <type>
...
```bash
/opt/homebrew/bin/claude-flow hook task-orchestrated --task-id <id>
...
```bash
/opt/homebrew/bin/claude-flow hook neural-trained --pattern <name>
...
```bash
/opt/homebrew/bin/claude-flow hook memory-sync --namespace <ns>
...
```bash
/opt/homebrew/bin/claude-flow hook session-start --session-id <id>
...
```bash
/opt/homebrew/bin/claude-flow hook session-restore --session-id <id>
...
Examples:
  /opt/homebrew/bin/claude-flow hook session-restore --session-id "swarm-20241019"
  /opt/homebrew/bin/claude-flow hook session-restore -s "feature-auth" --restore-memory
```
...
Examples:
  /opt/homebrew/bin/claude-flow hook session-end --session-id "dev-session-2024"
  /opt/homebrew/bin/claude-flow hook session-end -s "feature-auth" --export-metrics --generate-summary
  /opt/homebrew/bin/claude-flow hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
/opt/homebrew/bin/claude-flow hook notify --message <msg>
...
Examples:
  /opt/homebrew/bin/claude-flow hook notify -m "Task completed" --level info
  /opt/homebrew/bin/claude-flow hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "/opt/homebrew/bin/claude-flow hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "/opt/homebrew/bin/claude-flow hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "/opt/homebrew/bin/claude-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "/opt/homebrew/bin/claude-flow hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/opt/homebrew/bin/claude-flow hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "test -f '${tool.params.file_path%.js}.test.js' && ppnpm test '${tool.params.file_path%.js}.test.js'",
            "continueOnError": true
          }
        ]
      }
...
```javascript
// Hook command
/opt/homebrew/bin/claude-flow hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
/opt/homebrew/bin/claude-flow hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  /opt/homebrew/bin/claude-flow hook pre-edit --file "$FILE" --validate-syntax
  ...
  /opt/homebrew/bin/claude-flow hook post-edit --file "$FILE" --auto-format
done
...
/opt/homebrew/bin/claude-flow hook notify \
...
TRUTH_SCORE=$(/opt/homebrew/bin/claude-flow metrics score --format json | jq -r '.truth_score')
...
/opt/homebrew/bin/claude-flow hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
/opt/homebrew/bin/claude-flow hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
/opt/homebrew/bin/claude-flow hook post-edit \
  --file "api/auth.js" \
...
/opt/homebrew/bin/claude-flow hook post-task \
  --task-id "auth-api" \
  --analyze-performance \
  --store-decisions \
  --export-learnings
...
/opt/homebrew/bin/claude-flow hook session-restore \
...
/opt/homebrew/bin/claude-flow hook session-start --session-id "fullstack-feature"
...
# Backend work
/opt/homebrew/bin/claude-flow hook pre-edit --file "api/profile.js"
# ... implement backend ...
/opt/homebrew/bin/claude-flow hook post-edit \
  --file "api/profile.js" \
...
/opt/homebrew/bin/claude-flow hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
/opt/homebrew/bin/claude-flow hook post-edit \
...
/opt/homebrew/bin/claude-flow hook session-start --session-id "debug-memory-leak"
...
/opt/homebrew/bin/claude-flow hook pre-search --query "EventEmitter"
# ... search executes ...
/opt/homebrew/bin/claude-flow hook post-search \
  --query "EventEmitter" \
  --cache-results
...
/opt/homebrew/bin/claude-flow hook pre-task --description "Analyze code complexity"
# ... analysis ...
/opt/homebrew/bin/claude-flow hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  /opt/homebrew/bin/claude-flow hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  /opt/homebrew/bin/claude-flow hook post-edit \
    --file "$file" \
...
/opt/homebrew/bin/claude-flow hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .claude-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
/opt/homebrew/bin/claude-flow hook validate-config
```
...
- `/opt/homebrew/bin/claude-flow init --hooks` - Initialize hooks system
- `/opt/homebrew/bin/claude-flow hook --list` - List available hooks
- `/opt/homebrew/bin/claude-flow hook --test <hook>` - Test specific hook
- `/opt/homebrew/bin/claude-flow memory usage` - Manage memory
- `/opt/homebrew/bin/claude-flow agent spawn` - Spawn agents
- `/opt/homebrew/bin/claude-flow swarm init` - Initialize swarm
